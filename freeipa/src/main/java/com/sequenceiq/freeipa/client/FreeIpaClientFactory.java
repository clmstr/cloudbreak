...
public abstract class FreeIpaClientFactory <T> {
    ...
    public T getClient(Stack stack, InstanceMetaData instance)
        ...
        if (clusterProxyService.isCreateConfigForClusterProxy(stack)) {
            client = buildClientForClusterProxy(stack, instance);
        } else {
            client = buildClientForDirectConnect(stack, instance);
        }
        return client;
    }

    private T buildClientForClusterProxy(Stack stack, InstanceMetaData instanceMetaData)
            throws FreeIpaClientException, MalformedURLException {
        ...
        return buildFreeIpaClient(httpClientConfig, clusterProxyConfiguration.getClusterProxyPort(), clusterProxyPath, clusterProxyHeaders(),
                new FreeIpaClusterProxyErrorRpcListener());
    }

    private T buildClientForDirectConnect(Stack stack, InstanceMetaData instanceMetaData)
            throws FreeIpaClientException, MalformedURLException {
        ...
        return buildFreeIpaClient(httpClientConfig, gatewayPort, getDefaultBasePath(), Map.of(), null);
    }
    ...
    private T buildFreeIpaClient(HttpClientConfig clientConfig, int port, String basePath,
            Map<String, String> headers, RpcListener listener)
            throws FreeIpaClientException, MalformedURLException {
