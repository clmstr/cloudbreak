...
public class FreeipaChecker {
    ...
    private void checkNodeHealthReports(Stack stack, Set<InstanceMetaData> checkableInstances) {
        for (InstanceMetaData instanceMetaData : checkableInstances) {
            try {
                ...
                RPCResponse<HealthReport> meteringReportRPCResponse = checkedMeasure(() -> freeIpaNodeStatusService.nodeMeteringReport(stack, instanceMetaData),
                        LOGGER, ":::Auto sync::: FreeIPA metering report ran in {}ms");
                logReportResult(instanceMetaData, meteringReportRPCResponse, "metering");
    ...
    private void logReportResult(InstanceMetaData instanceMetaData, RPCResponse<HealthReport> meteringReportRPCResponse, String reportType) {
        if (isSuccessfulRequest(meteringReportRPCResponse)) {
            LOGGER.info("FreeIPA " + reportType + " reports for instance: [{}], report: [{}]", instanceMetaData.getInstanceId(), meteringReportRPCResponse.getResult());
        } else {
